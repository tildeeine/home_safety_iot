...
default_timer_duration = 3  # Test value. Set default timer duration (e.g., 3 minutes for testing)
timer_end_time = time.time() + default_timer_duration
latest_temperature = 0  # Store the latest temperature reading
temp_alert_status = 0  # Temperature alert status
...
def temperature():
    ...
    if request.method == 'POST':
        data = request.json
        latest_temperature = data.get('temperature', 0)
